...
const calculate = (board, winner) => {
  ...
  console.log("%cDAY 4B", "color:green");
  console.log(score * winner);
  console.log("\n");
};

let boardLeft = [...boards];
let lastBoardWinning;
let lastDrawWinning;
let found = false;

for (let j = 0; j < draws.length; j++) {
  boardLeft.forEach((pick) => {
    if (pick.value === draws[j]) {
      pick.draw = true;
      if (j > 4) {
        const row = boardLeft.filter(
          (boardItem) =>
            boardItem.board === pick.board && boardItem.x === pick.x
        );
        if (check(row)) {
          if (boardLeft.length > 1) {
            lastBoardWinning = row[0].board;
            lastDrawWinning = draws[j];
            boardLeft = boardLeft.filter(
              (boardItem) => boardItem.board !== pick.board
            );
          } else {
            found = true;
          ...
          const column = boardLeft.filter(
            (boardItem) =>
              boardItem.board === pick.board && boardItem.y === pick.y
          );
          if (check(column)) {
            if (boardLeft.length > 1) {
              lastBoardWinning = column[0].board;
              lastDrawWinning = draws[j];
              boardLeft = boardLeft.filter(
                (boardItem) => boardItem.board !== pick.board
              );
...
if (!found) {
  calculate(lastBoardWinning, lastDrawWinning);
}
